...
public class ExtractTitleData {
	...
	private static void readTitleTW() {
		try (
			BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream("tools/datafile/TW/C_Title.ini"), "UTF-8"));
		) {
	...
	private static void readMissionEU() {
		try (
			BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream("tools/datafile/TW/C_Mission.ini"), "UTF-8"));
		) {
			String line = reader.readLine();
			while (line != null) {
	...
	private static void readTitleEU() {
		try (
			BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream("tools/datafile/FR/T_Title.ini"), "Cp1252"));
		) {
			String line = reader.readLine();
			while (line != null) {
				if(line.matches("^[0-9]{4}\\|.*")) {
